...
# the slower prebuilt Vulkan backend. Run this once per release on a CUDA-capable
# Linux host, then attach the tarball to the GitHub release:
#
#   gh release upload <tag> dist/ggrun-linux-x86_64-cuda.tar.gz --repo raketenkater/ggrun
#
# install.sh already prefers an attached CUDA bundle over a source build.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
IK_REPO="${IK_LLAMA_REPO:-https://github.com/ikawrakow/ik_llama.cpp.git}"
IK_REF="${IK_LLAMA_REF:-main}"
...
( cd "$ROOT_DIR/go" && go build -trimpath -o ggrun ./cmd/ggrun )
...
if [[ -d "$IK_DIR/.git" ]]; then
    ...
    git clone --depth 1 --branch "$IK_REF" "$IK_REPO" "$IK_DIR"
fi
echo "==> Configuring + building llama-server (CUDA)"
cmake -S "$IK_DIR" -B "$IK_DIR/build" \
    -DCMAKE_BUILD_TYPE=Release -DGGML_CUDA=ON -DGGML_CUDA_FA_ALL_QUANTS=ON
...
echo "Attach it to the release so Linux NVIDIA installs skip the toolchain:"
echo "  gh release upload <tag> \"$OUT_DIR/$ASSET\" --repo raketenkater/ggrun"
