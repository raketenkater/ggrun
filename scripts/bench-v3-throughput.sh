...
python3 - "$PORT" "$(basename "$MODEL_ABS")" "$CONCURRENCY" "$REQUESTS" "$MAX_TOKENS" >"$JSON" <<'PY'
...
with open(summary_path, "w", encoding="utf-8") as f:
    ...
    f.write("| Parallel | Concurrency | Requests | Completion tok/s | Total tok/s | Avg latency s | Errors |\n")
    f.write("|---:|---:|---:|---:|---:|---:|---:|\n")
    f.write(
