...
type launchMemoryRecovery struct {
	...
	// the priced lever has correctly bowed out and the ceiling is the only
	// remaining way down.
	//
...
// rejectContextOutstripped records a rejection whose deficit was priced and found
// larger than one cut can cover. The caller decides this with
// contextDeficitOutstripsDevice, because only it holds the model and argv needed
// to price the deficit.
func (r *launchMemoryRecovery) rejectContextOutstripped(strategy *placement.Strategy) {
	if r == nil || strategy == nil || !strategy.ContextAuto || strategy.ContextSize <= 0 {
		return
	}
	if r.outstrippedContext == 0 || strategy.ContextSize < r.outstrippedContext {
		r.outstrippedContext = strategy.ContextSize
	}
}

// contextReclaimRequirement prices a measured VRAM deficit in context tokens on
// the device that failed, and reports whether the geometry was known well enough
// to price it at all.
//
// The two results answer different questions and the difference matters. "How
// many tokens would cover this?" and "do we know anything about this device's KV
// geometry?" were previously collapsed into a single 0 return, so a launch that
// knew the deficit exceeded the device's whole KV share was indistinguishable
// from one that knew nothing. Those want opposite responses: the first should
// descend, the second must stay conservative.
func contextReclaimRequirement(model *placement.ModelProfile, strategy *placement.Strategy, args []string, deficitMB, device int) (tokens int, priced bool) {
...
// contextDeficitOutstripsDevice reports that the deficit WAS priceable and is
// larger than one cut can cover while leaving a usable window -- the case where
// contextReclaimTokens correctly bows out, leaving the ceiling as the only
// descent.
//
// This is deliberately not "contextReclaimTokens returned 0". That value is also
// 0 for unknown geometry, where staying at the one-granule step is right
// (TestUnknownGeometryLeavesRecoveryAFallback). Here the geometry is known and
// the shortfall is simply bigger than the device's KV share, so a bounded
// descent is both safe and necessary: measured on a cold GLM-5.3-Flash probe
// 2026-09-15, five consecutive rounds surrendered 1,024 tokens each against a
// ~9,300 MiB deficit shrinking ~18 MiB per round.
func contextDeficitOutstripsDevice(model *placement.ModelProfile, strategy *placement.Strategy, args []string, deficitMB, device int) bool {
	tokens, priced := contextReclaimRequirement(model, strategy, args, deficitMB, device)
	if !priced || tokens <= 0 {
		return false
	}
	return strategy.ContextSize-tokens < contextRecoveryFloorTokens
}
...
//
// The returned value is a REQUIREMENT, not a step. oracleContextDropCoversDeficit
// uses it to judge whether a proposed drop covers the shortfall, so it must never
// be clamped to something smaller than the deficit actually needs.
...
func (r *launchMemoryRecovery) automaticContextCeiling() int {
	...
	// A priced-but-uncoverable rejection descends by the same fraction the priced
	// lever already bounds itself by, so this adds no step size the system has not
	// already reasoned about. Every guard the one-granule step relied on still
	// holds: it only ever lowers the ceiling, never drops below the usable-window
	// floor, and never goes under a context an exact preflight has proven fits --
	// spending proven context to chase an unproven one is the trade
	// acceptedContext exists to prevent.
	if r.outstrippedContext > 1 {
		candidate := r.outstrippedContext - r.outstrippedContext/contextRecoveryStepDivisor
